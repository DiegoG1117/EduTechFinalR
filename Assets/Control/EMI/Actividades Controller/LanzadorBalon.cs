...
public class LanzadorBalon : MonoBehaviour
{
    ...
    public Transform JuanPosicion; // El punto inicial del balón en la parábola
    ...
    void Start()
    {
        Vector3 posicionActual = balonJ.position;
        nuevaPosicionY = CE.AlturaJ;
        nuevaPosicionx = 13.728f - CE.DistanciaJ;
        ...
        balonJ.position = posicionActual;
        puntoInicial.position = posicionActual;
        JuanPosicion.position = posicionActual;
    ...
    void LanzarBalon()
    {
        ...
        float anguloRad = Mathf.Deg2Rad * CE.AnguloJ;
        float velocidadX = CE.VelocidadInicialJ * Mathf.Cos(anguloRad);
        float velocidadY = CE.VelocidadInicialJ * Mathf.Sin(anguloRad);
